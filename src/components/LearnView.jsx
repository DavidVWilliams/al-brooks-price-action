// Version: v5.3 - Macro Phase Rail & Dynamic LabRegistry (Tiers 0-5 Full Curriculum)
import { useState, useMemo } from 'react';
import data from '../data/curriculumData.js';
...
const TIERS = [
  ...
  { id: 'tier4', label: '4. Advanced Structures' },
...
export default function LearnView() {
  ...
  const tierModules = useMemo(() => {
    return allModules.filter((m) => m.id?.startsWith(activeTier));
  }, [allModules, activeTier]);

  const [selectedModule, setSelectedModule] = useState(
    tierModules[0] || allModules[0] || {}
  );
  ...
  const handleSelectTier = (tierId) => {
    ...
    const firstInTier = allModules.find((m) => m.id?.startsWith(tierId));
    if (firstInTier) {
      handleSelectModule(firstInTier);
    }
  };
  ...
  // Dynamically resolve interactive lab from registry
  ...
  return (
    <div className="absolute inset-0 flex flex-col bg-slate-950 text-slate-100">
      ...
      {/* TOP MACRO PHASE RAIL (TIERS 0 - 5) */}
      <nav className="border-b border-slate-800 bg-slate-900/80 px-4 py-2 flex items-center gap-2 overflow-x-auto shrink-0 [&::-webkit-scrollbar]:hidden">
        {TIERS.map((t) => (
          <button
            key={t.id}
            onClick={() => handleSelectTier(t.id)}
            className={`px-3 py-1.5 rounded-lg text-xs font-mono font-semibold uppercase tracking-wider whitespace-nowrap transition-colors ${
              activeTier === t.id
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        {/* Sidebar Navigation */}
        <aside className="w-full md:w-80 border-b md:border-b-0 md:border-r border-slate-800 bg-slate-900/40 flex flex-col shrink-0 h-44 md:h-full">
          <div className="p-3 border-b border-slate-800 bg-slate-900/85 flex items-center justify-between">
            ...
            <span className="text-[10px] font-mono text-blue-400 bg-blue-950 px-2 py-0.5 rounded border border-blue-900/60">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1 [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-800 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-slate-700 transition-colors">
            {tierModules.map((mod) => (
              <button
                ...
                className={`w-full px-3 py-2.5 rounded-lg text-xs md:text-sm text-left transition-colors flex items-center justify-between ${
          ...
          <section className="p-6 md:p-12 flex flex-col max-w-4xl mx-auto w-full">
            ...
            {selectedModule.type === 'comprehensive_lesson' && selectedModule.sections && (
              <div className="space-y-10 text-left">
                ...
                {/* DYNAMIC INTERACTIVE CAPSTONE LAB (FROM LABREGISTRY) */}
                {ActiveLabComponent && (
