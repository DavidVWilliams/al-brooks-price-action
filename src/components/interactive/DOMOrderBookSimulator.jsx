// Version: v1.0 - Tier 0 Interactive DOM Order Book & Indicator Lag Simulator
// Simulates order book matching engine, liquidity consumption, and indicator latency.
...
export default function DOMOrderBookSimulator() {
  ...
  // Helper to record price updates
  ...
  // Aggressive Market Buy: sweeps Ask side
  ...
  // Aggressive Market Sell: sweeps Bid side
  ...
  // Institutional Limit Iceberg Bid
  ...
  // Geometry calculation for dynamic candle
  const minP = 5018.75;
  const maxP = 5021.50;
  const svgH = 180;
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 md:p-6 shadow-2xl space-y-6 text-slate-100">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
                  ...
                  >
                    ...
                    <div className="w-16 flex items-center gap-1.5">
                      {lvl.bidQty > 0 && (
                        <>
                          <div
                            className="h-2 bg-emerald-500/50 rounded-sm"
                            style={{ width: `${Math.min(100, (lvl.bidQty / 600) * 45)}px` }}
                          />
                    ...
                    {/* Price */}
                    <span className={isCurrent ? 'text-white font-bold' : 'text-slate-300'}>
                      {lvl.price.toFixed(2)}
                    </span>
                      ...
                      {lvl.askQty > 0 && (
                        <>
                          ...
                          <div
                            ...
                            style={{ width: `${Math.min(100, (lvl.askQty / 600) * 45)}px` }}
                          />
                        </>
                      )}
                    </div>
          ...
          <svg className="w-full h-44 select-none" viewBox="0 0 160 180">
            ...
            {[5021.0, 5020.5, 5020.0, 5019.5].map((p) => (
              <g key={p}>
                ...
                <text x="155" y={getY(p) + 3} fill="#475569" fontSize="7" fontFamily="monospace" textAnchor="end">
                  {p.toFixed(2)}
                </text>
              </g>
            ))}
        ...
        {/* PANE 3: LAGGING INDICATOR & ORDER FLOW TELEMETRY */}
            ...
            <div className="p-2.5 bg-slate-900 rounded-lg border border-slate-800 space-y-1">
              ...
              <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, Math.max(0, oscillatorValue))}%` }}
                />
              </div>
              <span className="text-[10px] text-slate-500 block pt-0.5">
                Latency: Average 7 to 14 bars behind immediate order flow.
              </span>
            </div>
          </div>

          <div className="p-3 bg-blue-950/30 border-l-2 border-blue-500 rounded-r text-xs text-slate-300 leading-relaxed">
            <strong className="text-blue-400 block mb-1 font-mono uppercase text-[10px]">Al Brooks Principle</strong>
            "Price moves solely because aggressive market orders consume resting limit liquidity. Oscillators cannot see who is trapped until long after the move is over."
          </div>
      ...
      <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={() => handleMarketBuy(220)}
              className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded text-xs font-mono font-semibold transition-colors"
            >
              + Sweep Asks (Market Buy 220)
            </button>
            <button
              onClick={() => handleMarketSell(220)}
              className="px-3.5 py-1.5 bg-rose-600 hover:bg-rose-500 text-white rounded text-xs font-mono font-semibold transition-colors"
            >
              - Sweep Bids (Market Sell 220)
            </button>
            <button
              onClick={handleAddIcebergBid}
              className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-blue-300 rounded border border-blue-900/50 text-xs font-mono transition-colors"
            >
              + Place Passive Iceberg Bid (600 @ 5019.50)
            </button>
          </div>

          <button
            onClick={handleReset}
            className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-slate-400 rounded text-xs font-mono border border-slate-800 transition-colors"
          >
            ↺ Reset Auction
          </button>
        </div>
        ...
        <div className="text-xs font-mono text-slate-400 bg-slate-900/60 px-3 py-2 rounded border border-slate-800/80 truncate">
          <span className="text-blue-400 font-semibold">Feed: </span>
          {log}
        </div>
      </div>
