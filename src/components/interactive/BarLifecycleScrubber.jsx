...
export default function BarLifecycleScrubber() {
  ...
  const calculateBarState = (s) => {
    ...
    if (s <= 30) {
      ...
      currentPrice = (openPrice + 0.75) - progress * 6.75;
      ...
      highPrice = openPrice + 0.75;
      lowPrice = 5014.00;
      currentPrice = 5014.00 + Math.sin(progress * Math.PI * 3) * 0.75 + progress * 1.5;
      ...
      highPrice = openPrice + 0.75;
      lowPrice = 5014.00;
      currentPrice = 5015.50 + progress * 5.00;
      ...
      lowPrice = 5014.00;
      highPrice = openPrice + 0.75 + progress * 2.25;
      currentPrice = 5020.50 + progress * 2.00;
    ...
    return { currentPrice, highPrice, lowPrice, totalRange, closeLocationPct, phase, narrative, retailBias };
  };
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 md:p-6 shadow-2xl space-y-6 text-slate-100">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base md:text-lg font-bold text-white">
              The 300-Second Bar Lifecycle & Dominance of the Close
            </h3>
          </div>
          <p className="text-xs text-slate-400 mt-1">
            Drag the time slider or play the simulation to watch how institutions use the final 30 seconds to trap breakout traders.
          </p>
        </div>
      ...
      <div className="bg-blue-950/20 border border-blue-900/50 rounded-lg p-4 space-y-2">
        ...
        <ol className="list-decimal pl-5 text-sm text-slate-300 space-y-1">
          <li>Click the <strong className="text-rose-400">1:00 Flush</strong> preset button below. Notice the massive red bar and the "Heavy Bearish FOMO" reading.</li>
          <li>Click the <strong className="text-emerald-400">4:50 Squeeze</strong> button. Watch the bar instantly transform from a breakdown into a bullish trap.</li>
          <li><strong>Takeaway:</strong> Never execute a trade based on how a bar looks in the middle of its lifespan. The close overrides everything.</li>
        </ol>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
          ...
          <svg className="w-full h-56 select-none" viewBox="0 0 240 220">
            {[5024, 5022, 5020, 5018, 5016, 5014].map((p) => (
              <g key={p}>
                ...
                <text x="235" y={getY(p) + 3} fill="#475569" fontSize="8" fontFamily="monospace" textAnchor="end">{p.toFixed(2)}</text>
              </g>
            ))}
            <line x1="20" y1={openY} x2="210" y2={openY} stroke="#64748b" strokeWidth="1" strokeDasharray="4 4" />
            <text x="25" y={openY - 4} fill="#94a3b8" fontSize="8" fontFamily="monospace">Open: 5020.00</text>
            <line x1="120" y1={highY} x2="120" y2={lowY} stroke={isBull ? '#10b981' : '#f43f5e'} strokeWidth="2" />
            <rect x="95" y={bodyTop} width="50" height={bodyHeight} fill={isBull ? '#065f46' : '#881337'} stroke={isBull ? '#10b981' : '#f43f5e'} strokeWidth="2" rx="2" />
            ...
            <text x="180" y={currentY + 3} fill={isBull ? '#34d399' : '#fb7185'} fontSize="9" fontFamily="monospace" fontWeight="bold">{state.currentPrice.toFixed(2)}</text>
          </svg>
        ...
        <div className="md:col-span-7 space-y-3.5">
          ...
          <div className="p-3 bg-slate-950/80 rounded-lg border border-slate-800 space-y-1">
            <span className="text-[10px] font-mono uppercase tracking-wider text-blue-400 font-bold block">Current Auction State</span>
            ...
            <p className="text-xs text-slate-300 leading-relaxed pt-1">{state.narrative}</p>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 bg-slate-950/60 rounded-lg border border-slate-800 space-y-1">
              <span className="text-[10px] font-mono uppercase text-slate-400 font-semibold block">Retail Crowd Posture</span>
              <span className={`text-xs font-bold ${seconds <= 110 ? 'text-rose-400' : seconds >= 270 ? 'text-rose-300' : 'text-amber-400'}`}>{state.retailBias}</span>
            </div>
            <div className="p-3 bg-slate-950/60 rounded-lg border border-slate-800 space-y-1">
              <span className="text-[10px] font-mono uppercase text-slate-400 font-semibold block">Closing Footprint</span>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold font-mono text-white">{state.closeLocationPct}%</span>
                <span className={`text-[11px] font-mono px-1.5 py-0.5 rounded font-semibold ${
                  state.closeLocationPct >= 80 ? 'bg-emerald-950 text-emerald-400 border border-emerald-800' : state.closeLocationPct <= 20 ? 'bg-rose-950 text-rose-400 border border-rose-800' : 'bg-slate-800 text-slate-300'
                }`}>
                  {state.closeLocationPct >= 80 ? 'Top 20%' : state.closeLocationPct <= 20 ? 'Bottom 20%' : 'Middle 50%'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="p-3.5 bg-blue-950/30 border-l-4 border-blue-500 rounded-r-lg space-y-1">
            <span className="text-[10px] font-mono uppercase tracking-wider text-blue-400 font-bold block">Core Al Brooks Axiom</span>
